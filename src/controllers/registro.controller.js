...
export const getRegistros = async (req, res, next) => {
  try {
    ...
    // Admin: ve todos los registros sin restricción
  ...
  try {
    ...
    if (req.isAdmin) {
      result = await pool.query(
        "SELECT DISTINCT unidad FROM registros WHERE unidad IS NOT NULL ORDER BY unidad",
      );
    } else {
      result = await pool.query(
        "SELECT DISTINCT unidad FROM registros WHERE unidad IS NOT NULL AND dirigente_ci = $1 ORDER BY unidad",
        [req.userCI],
      );
    }
...
export const createRegistro = async (req, res, next) => {
  ...
  try {
    ...
    let scoutCheck;
    if (req.isAdmin) {
      scoutCheck = await pool.query("SELECT * FROM scouts WHERE ci = $1", [
        scout_ci,
      ]);
    } else {
      scoutCheck = await pool.query(
        "SELECT * FROM scouts WHERE ci = $1 AND dirigente_ci = $2",
        [scout_ci, req.userCI],
      );
    }
    ...
    if (scoutCheck.rowCount === 0) {
      return res.status(404).json({
        message: "Scout no encontrado o no pertenece a este dirigente",
      });
    }
...
export const deleteRegistro = async (req, res, next) => {
  try {
    ...
    let result;
    if (req.isAdmin) {
      result = await pool.query("DELETE FROM registros WHERE id = $1", [id]);
    } else {
      result = await pool.query(
