...
export const useAuth = () =>{
    ...
    if(!context){
        throw new Error ('useAuth error: El hook useAuth debe usarse dentro de un AuthProvider') // Mensaje más descriptivo
    }
    return context
}
...
export function AuthProvider ({children}){
    ...
    const signin = async (data) => {
        ...
        try {
            ...
            setUser(res.data);
            setIsAuth(true);
            return res.data; // Devuelve los datos del usuario en caso de éxito
        } catch (error) {
        ...
        const checkLogin = async () => {
            ...
            if (token) {
                try {
                    ...
                    console.log("Error al verificar sesión con token:", err.response || err);
                    setUser(null);
                    setIsAuth(false);
